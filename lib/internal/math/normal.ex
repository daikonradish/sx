defmodule Internal.Math.Normal do
  ...
  def ppnd16(p) do
    # accepts only values in (0, 1)
    # otherwise you will error when trying to call log later.
    q = p - @half

    case abs(q) <= @split1 do
      true ->
        handle_case_1(q, @const1 - q * q)

      false ->
        rr = if q < 0, do: p, else: @one - p
        r = Math.sqrt(-Math.log(rr))

        ppnd =
          case r <= @split2 do
            true -> handle_case_2(r - @const2)
            false -> handle_case_3(r - @split2)
          end

        if q < 0, do: -ppnd, else: ppnd
    end
  end

  defp handle_case_1(q, r) do
    q *
  ...
  defp handle_case_3(r) do
    (((((((@e7 * r + @e6) * r + @e5) * r + @e4) * r +
          @e3) * r + @e2) * r + @e1) * r + @e0) /
      (((((((@f7 * r + @f6) * r + @f5) * r + @f4) *
            r + @f3) * r + @f2) * r + @f1) * r + @one)
  end
end
